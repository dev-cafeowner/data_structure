...
int top;                    // 스택의 최상단 위치를 가리키는 변수 (-1은 비어있음을 의미)

// 스택 초기화하는 함수
void init()
{
    // top을 -1로 초기화 하여 스택을 초기화
    top = -1;
}
...
{
    // 가득 찼을 때 데이터를 넣으려 하면 에러 발생 (Stack Overflow)
    if (is_full())
    {
...
element pop()
{
    // 비어있는데 데이터를 꺼내려 하면 프로그램 강제 종료 (Stack Underflow)
    if (is_empty())
    {
        fprintf(stderr, "스택 공백 에러\n");
        // 비상 정지: 비정상 종료(1) 상태 코드를 OS에 전달
        exit(1);
    }
